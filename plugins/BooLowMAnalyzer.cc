...
BooLowMAnalyzer::BooLowMAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  gSystem->Setenv("LHAPATH","/uscms_data/d2/jengbou/Generators/LHAPDFLIB/share/lhapdf/PDFsets/");
  ...
  for(int i=0; i < nmembers; i++){
    TString suffix = "PDF_";
    suffix+=i;
    hmass_->Init("MassPDF","M3",suffix);
    hmass_->Init("MassPDF","M3prime",suffix);
  } 
}
...
{	
  ...
  if (genTop && genTopBar) {
	...
	if ( DeltaR<reco::Candidate>()(*genHadWp, *genHadWq ) < theminGenDeltaR )
	  theminGenDeltaR = DeltaR<reco::Candidate>()(*genHadWp, *genHadWq );
	if ( DeltaR<reco::Candidate>()(*genHadWp, *genHadb ) < theminGenDeltaR ) 
	  theminGenDeltaR = DeltaR<reco::Candidate>()(*genHadWp, *genHadb );
	if ( DeltaR<reco::Candidate>()(*genHadWp, *genLepb ) < theminGenDeltaR ) 
	  theminGenDeltaR = DeltaR<reco::Candidate>()(*genHadWp, *genLepb );
	if ( DeltaR<reco::Candidate>()(*genHadWq, *genHadb ) < theminGenDeltaR ) 
	  theminGenDeltaR = DeltaR<reco::Candidate>()(*genHadWq, *genHadb );
	if ( DeltaR<reco::Candidate>()(*genHadWq, *genLepb ) < theminGenDeltaR ) 
	  theminGenDeltaR = DeltaR<reco::Candidate>()(*genHadWq, *genLepb );
	if ( DeltaR<reco::Candidate>()(*genHadb, *genLepb ) < theminGenDeltaR ) 
	  theminGenDeltaR = DeltaR<reco::Candidate>()(*genHadb, *genLepb );
	...
	// 	} else {
	// 	  edm::LogWarning ( "BooLowMAnalyzer" ) << "Not Semileptonic decay";
	//       }
  ...
  if (NgoodJets >=4 ) {
    ...
    if (debug) std::cout << "HadronicTop mass (M3) = " << hadTopP4.M() << std::endl;
    if (debug) std::cout << "runPDF for M3 and M3' = " << runPDF << std::endl;
	...
	if(debug){
	  std::cout<<"Weights = "<<weights[i]/weights[0]<<"; ";
	  std::cout<<TString("HadronicTop_mass_M3_")+suffix<<std::endl;
	}
	hmass_->Fill1d(TString("HadronicTop_mass_M3_")+suffix, hadTopP4.M(), ww);
      }
    }
    
    // number of b-tagged jets MediumOP
    int nbtags =0;
  ...
  if ( found_goodMET) {
	...
	hmass_->Fill2d(TString("HadTop_vs_HadW")+"_cut0", hadTopP4.M(), hadWP4.M());
      ...
      if (debug){
	std::cout << "got best combination using chi^2 sorting" << std::endl;
	std::cout << "HadronicTop mass (M3prime) = " << hadTopP4.M() << std::endl;
	// debug
	//bestCombo.Print();
      }
	...
	for(int i=0; i < nmembers; i++){
	  ...
	  if(debug){
	    std::cout<<"Weights = "<<ww<< "; ";
	    std::cout<<TString("HadronicTop_mass_M3prime_")+suffix<<std::endl;
	  }
	  hmass_->Fill1d(TString("HadronicTop_mass_M3prime_")+suffix, hadTopP4.M(), ww);
	}
      }
